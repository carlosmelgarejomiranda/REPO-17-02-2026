...
export const Navbar = ({ user, onLoginClick, onLogout, language, setLanguage, t }) => {
  ...
  // UGC submenu based on user role
  const getUGCLink = () => {
    if (!user) return '/ugc/select-role';
    if (user.role === 'creator') return '/ugc/creator/dashboard';
    if (user.role === 'brand') return '/ugc/brand/dashboard';
    return '/ugc/select-role';
  };
